 ...
 */
(function(root,factory){if(typeof define==='function'&&define.amd){define(function(){root.Reveal=factory();return root.Reveal})}else if(typeof exports==='object'){module.exports=factory()}else{root.Reveal=factory()}}(this,function(){'use strict';var Reveal;var VERSION='3.6.0';var SLIDES_SELECTOR='.slides section',HORIZONTAL_SLIDES_SELECTOR='.slides>section',VERTICAL_SLIDES_SELECTOR='.slides>section.present>section',HOME_SLIDE_SELECTOR='.slides>section:first-of-type',UA=navigator.userAgent,config={width:960,height:700,margin:0.04,minScale:0.2,maxScale:2.0,controls:!0,controlsTutorial:!0,controlsLayout:'bottom-right',controlsBackArrows:'faded',progress:!0,slideNumber:!1,showSlideNumber:'all',history:!1,keyboard:!0,keyboardCondition:null,overview:!0,center:!0,touch:!0,loop:!1,rtl:!1,shuffle:!1,fragments:!0,embedded:!1,help:!0,pause:!0,showNotes:!1,autoPlayMedia:null,autoSlide:0,autoSlideStoppable:!0,autoSlideMethod:null,mouseWheel:!1,rollingLinks:!1,hideAddressBar:!0,previewLinks:!1,postMessage:!0,postMessageEvents:!1,focusBodyOnPageVisibilityChange:!0,transition:'slide',transitionSpeed:'default',backgroundTransition:'fade',parallaxBackgroundImage:'',parallaxBackgroundSize:'',parallaxBackgroundHorizontal:null,parallaxBackgroundVertical:null,viewDistance:3,display:'block',dependencies:[]},initialized=!1,loaded=!1,overview=!1,overviewSlideWidth=null,overviewSlideHeight=null,indexh,indexv,previousSlide,currentSlide,previousBackground,hasNavigatedRight=!1,hasNavigatedDown=!1,state=[],scale=1,slidesTransform={layout:'',overview:''},dom={},features={},isMobileDevice,isChrome,lastMouseWheelStep=0,writeURLTimeout=0,eventsAreBound=!1,autoSlide=0,autoSlidePlayer,autoSlideTimeout=0,autoSlideStartTime=-1,autoSlidePaused=!1,touch={startX:0,startY:0,startSpan:0,startCount:0,captured:!1,threshold:40},keyboardShortcuts={'N  ,  SPACE':'Next slide','P':'Previous slide','&#8592;  ,  H':'Navigate left','&#8594;  ,  L':'Navigate right','&#8593;  ,  K':'Navigate up','&#8595;  ,  J':'Navigate down','Home':'First slide','End':'Last slide','B  ,  .':'Pause','F':'Fullscreen','ESC, O':'Slide overview'};function initialize(options){if(initialized===!0)return;initialized=!0;checkCapabilities();if(!features.transforms2d&&!features.transforms3d){document.body.setAttribute('class','no-transforms');var images=toArray(document.getElementsByTagName('img')),iframes=toArray(document.getElementsByTagName('iframe'));var lazyLoadable=images.concat(iframes);for(var i=0,len=lazyLoadable.length;i<len;i++){var element=lazyLoadable[i];if(element.getAttribute('data-src')){element.setAttribute('src',element.getAttribute('data-src'));element.removeAttribute('data-src')}}
return}
dom.wrapper=document.querySelector('.reveal');dom.slides=document.querySelector('.reveal .slides');window.addEventListener('load',layout,!1);var query=Reveal.getQueryHash();if(typeof query.dependencies!=='undefined')delete query.dependencies;extend(config,options);extend(config,query);hideAddressBar();load()}
...
function start(){loaded=!0;setupDOM();setupPostMessage();setupScrollPrevention();resetVerticalSlides();configure();readURL();updateBackground(!0);setTimeout(function(){dom.slides.classList.remove('no-transition');dom.wrapper.classList.add('ready');dispatchEvent('ready',{'indexh':indexh,'indexv':indexv,'currentSlide':currentSlide})},1)}
...
dom.background=createSingletonNode(dom.wrapper,'div','backgrounds',null);dom.progress=createSingletonNode(dom.wrapper,'div','progress','<span></span>');dom.progressbar=dom.progress.querySelector('span');dom.controls=createSingletonNode(dom.wrapper,'aside','controls','<button class="navigate-left" aria-label="previous slide"><div class="controls-arrow"></div></button>'+'<button class="navigate-right" aria-label="next slide"><div class="controls-arrow"></div></button>'+'<button class="navigate-up" aria-label="above slide"><div class="controls-arrow"></div></button>'+'<button class="navigate-down" aria-label="below slide"><div class="controls-arrow"></div></button>');dom.slideNumber=createSingletonNode(dom.wrapper,'div','slide-number','');dom.speakerNotes=createSingletonNode(dom.wrapper,'div','speaker-notes',null);dom.speakerNotes.setAttribute('data-prevent-swipe','');dom.speakerNotes.setAttribute('tabindex','0');createSingletonNode(dom.wrapper,'div','pause-overlay',null);dom.wrapper.setAttribute('role','application');dom.controlsLeft=toArray(document.querySelectorAll('.navigate-left'));dom.controlsRight=toArray(document.querySelectorAll('.navigate-right'));dom.controlsUp=toArray(document.querySelectorAll('.navigate-up'));dom.controlsDown=toArray(document.querySelectorAll('.navigate-down'));dom.controlsPrev=toArray(document.querySelectorAll('.navigate-prev'));dom.controlsNext=toArray(document.querySelectorAll('.navigate-next'));dom.controlsRightArrow=dom.controls.querySelector('.navigate-right');dom.controlsDownArrow=dom.controls.querySelector('.navigate-down');dom.statusDiv=createStatusDiv()}
...
function createBackgrounds(){dom.background.innerHTML='';dom.background.classList.add('no-transition');toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR)).forEach(function(slideh){var backgroundStack=createBackground(slideh,dom.background);toArray(slideh.querySelectorAll('section')).forEach(function(slidev){createBackground(slidev,backgroundStack);backgroundStack.classList.add('stack')})});if(config.parallaxBackgroundImage){dom.background.style.backgroundImage='url("'+config.parallaxBackgroundImage+'")';dom.background.style.backgroundSize=config.parallaxBackgroundSize;setTimeout(function(){dom.wrapper.classList.add('has-parallax-background')},1)}
...
if(config.pause===!1){resume()}
if(config.showNotes){dom.speakerNotes.setAttribute('data-layout',typeof config.showNotes==='string'?config.showNotes:'inline')}
if(config.mouseWheel){document.addEventListener('DOMMouseScroll',onDocumentMouseScroll,!1);document.addEventListener('mousewheel',onDocumentMouseScroll,!1)}
else{document.removeEventListener('DOMMouseScroll',onDocumentMouseScroll,!1);document.removeEventListener('mousewheel',onDocumentMouseScroll,!1)}
...
if(numberOfSlides>1&&config.autoSlide&&config.autoSlideStoppable&&features.canvas&&features.requestAnimationFrame){autoSlidePlayer=new Playback(dom.wrapper,function(){return Math.min(Math.max((Date.now()-autoSlideStartTime)/autoSlide,0),1)});autoSlidePlayer.on('click',onAutoSlidePlayerClick);autoSlidePaused=!1}
if(config.fragments===!1){toArray(dom.slides.querySelectorAll('.fragment')).forEach(function(element){element.classList.add('visible');element.classList.remove('current-fragment')})}
var slideNumberDisplay='none';if(config.slideNumber){if(config.showSlideNumber==='all'){slideNumberDisplay='block'}
else if(config.showSlideNumber==='speaker'&&isSpeakerNotes()){slideNumberDisplay='block'}}
dom.slideNumber.style.display=slideNumberDisplay;sync()}
function addEventListeners(){eventsAreBound=!0;window.addEventListener('hashchange',onWindowHashChange,!1);window.addEventListener('resize',onWindowResize,!1);if(config.touch){dom.wrapper.addEventListener('touchstart',onTouchStart,!1);dom.wrapper.addEventListener('touchmove',onTouchMove,!1);dom.wrapper.addEventListener('touchend',onTouchEnd,!1);if(window.navigator.pointerEnabled){dom.wrapper.addEventListener('pointerdown',onPointerDown,!1);dom.wrapper.addEventListener('pointermove',onPointerMove,!1);dom.wrapper.addEventListener('pointerup',onPointerUp,!1)}
else if(window.navigator.msPointerEnabled){dom.wrapper.addEventListener('MSPointerDown',onPointerDown,!1);dom.wrapper.addEventListener('MSPointerMove',onPointerMove,!1);dom.wrapper.addEventListener('MSPointerUp',onPointerUp,!1)}}
if(config.keyboard){document.addEventListener('keydown',onDocumentKeyDown,!1);document.addEventListener('keypress',onDocumentKeyPress,!1)}
if(config.progress&&dom.progress){dom.progress.addEventListener('click',onProgressClicked,!1)}
...
if(visibilityChange){document.addEventListener(visibilityChange,onPageVisibilityChange,!1)}}
var pointerEvents=['touchstart','click'];if(UA.match(/android/gi)){pointerEvents=['touchstart']}
pointerEvents.forEach(function(eventName){dom.controlsLeft.forEach(function(el){el.addEventListener(eventName,onNavigateLeftClicked,!1)});dom.controlsRight.forEach(function(el){el.addEventListener(eventName,onNavigateRightClicked,!1)});dom.controlsUp.forEach(function(el){el.addEventListener(eventName,onNavigateUpClicked,!1)});dom.controlsDown.forEach(function(el){el.addEventListener(eventName,onNavigateDownClicked,!1)});dom.controlsPrev.forEach(function(el){el.addEventListener(eventName,onNavigatePrevClicked,!1)});dom.controlsNext.forEach(function(el){el.addEventListener(eventName,onNavigateNextClicked,!1)})})}
function removeEventListeners(){eventsAreBound=!1;document.removeEventListener('keydown',onDocumentKeyDown,!1);document.removeEventListener('keypress',onDocumentKeyPress,!1);window.removeEventListener('hashchange',onWindowHashChange,!1);window.removeEventListener('resize',onWindowResize,!1);dom.wrapper.removeEventListener('touchstart',onTouchStart,!1);dom.wrapper.removeEventListener('touchmove',onTouchMove,!1);dom.wrapper.removeEventListener('touchend',onTouchEnd,!1);if(window.navigator.pointerEnabled){dom.wrapper.removeEventListener('pointerdown',onPointerDown,!1);dom.wrapper.removeEventListener('pointermove',onPointerMove,!1);dom.wrapper.removeEventListener('pointerup',onPointerUp,!1)}
else if(window.navigator.msPointerEnabled){dom.wrapper.removeEventListener('MSPointerDown',onPointerDown,!1);dom.wrapper.removeEventListener('MSPointerMove',onPointerMove,!1);dom.wrapper.removeEventListener('MSPointerUp',onPointerUp,!1)}
if(config.progress&&dom.progress){dom.progress.removeEventListener('click',onProgressClicked,!1)}['touchstart','click'].forEach(function(eventName){dom.controlsLeft.forEach(function(el){el.removeEventListener(eventName,onNavigateLeftClicked,!1)});dom.controlsRight.forEach(function(el){el.removeEventListener(eventName,onNavigateRightClicked,!1)});dom.controlsUp.forEach(function(el){el.removeEventListener(eventName,onNavigateUpClicked,!1)});dom.controlsDown.forEach(function(el){el.removeEventListener(eventName,onNavigateDownClicked,!1)});dom.controlsPrev.forEach(function(el){el.removeEventListener(eventName,onNavigatePrevClicked,!1)});dom.controlsNext.forEach(function(el){el.removeEventListener(eventName,onNavigateNextClicked,!1)})})}
function extend(a,b){for(var i in b){a[i]=b[i]}
return a}
function toArray(o){return Array.prototype.slice.call(o)}
function deserialize(value){if(typeof value==='string'){if(value==='null')return null;else if(value==='true')return!0;else if(value==='false')return!1;else if(value.match(/^-?[\d\.]+$/))return parseFloat(value)}
return value}
...
return height}
function hideAddressBar(){if(config.hideAddressBar&&isMobileDevice){window.addEventListener('load',removeAddressBar,!1);window.addEventListener('orientationchange',removeAddressBar,!1)}}
function removeAddressBar(){setTimeout(function(){window.scrollTo(0,1)},10)}
function dispatchEvent(type,args){var event=document.createEvent('HTMLEvents',1,2);event.initEvent(type,!0,!0);extend(event,args);dom.wrapper.dispatchEvent(event);if(config.postMessageEvents&&window.parent!==window.self){window.parent.postMessage(JSON.stringify({namespace:'reveal',eventName:type,state:getState()}),'*')}}
...
function enablePreviewLinks(selector){var anchors=toArray(document.querySelectorAll(selector?selector:'a'));anchors.forEach(function(element){if(/^(http|www)/gi.test(element.getAttribute('href'))){element.addEventListener('click',onPreviewLinkClicked,!1)}})}
function disablePreviewLinks(selector){var anchors=toArray(document.querySelectorAll(selector?selector:'a'));anchors.forEach(function(element){if(/^(http|www)/gi.test(element.getAttribute('href'))){element.removeEventListener('click',onPreviewLinkClicked,!1)}})}
function showPreview(url){closeOverlay();dom.overlay=document.createElement('div');dom.overlay.classList.add('overlay');dom.overlay.classList.add('overlay-preview');dom.wrapper.appendChild(dom.overlay);dom.overlay.innerHTML=['<header>','<a class="close" href="#"><span class="icon"></span></a>','<a class="external" href="'+url+'" target="_blank"><span class="icon"></span></a>','</header>','<div class="spinner"></div>','<div class="viewport">','<iframe src="'+url+'"></iframe>','<small class="viewport-inner">','<span class="x-frame-error">Unable to load iframe. This is likely due to the site\'s policy (x-frame-options).</span>','</small>','</div>'].join('');dom.overlay.querySelector('iframe').addEventListener('load',function(event){dom.overlay.classList.add('loaded')},!1);dom.overlay.querySelector('.close').addEventListener('click',function(event){closeOverlay();event.preventDefault()},!1);dom.overlay.querySelector('.external').addEventListener('click',function(event){closeOverlay()},!1);setTimeout(function(){dom.overlay.classList.add('visible')},1)}
...
return 0}
function activateOverview(){if(config.overview&&!isOverview()){overview=!0;dom.wrapper.classList.add('overview');dom.wrapper.classList.remove('overview-deactivating');if(features.overviewTransitions){setTimeout(function(){dom.wrapper.classList.add('overview-animated')},1)}
cancelAutoSlide();dom.slides.appendChild(dom.background);toArray(dom.wrapper.querySelectorAll(SLIDES_SELECTOR)).forEach(function(slide){if(!slide.classList.contains('stack')){slide.addEventListener('click',onOverviewSlideClicked,!0)}});var margin=70;var slideSize=getComputedSlideSize();overviewSlideWidth=slideSize.width+margin;overviewSlideHeight=slideSize.height+margin;if(config.rtl){overviewSlideWidth=-overviewSlideWidth}
...
function deactivateOverview(){if(config.overview){overview=!1;dom.wrapper.classList.remove('overview');dom.wrapper.classList.remove('overview-animated');dom.wrapper.classList.add('overview-deactivating');setTimeout(function(){dom.wrapper.classList.remove('overview-deactivating')},1);dom.wrapper.appendChild(dom.background);toArray(dom.wrapper.querySelectorAll(SLIDES_SELECTOR)).forEach(function(slide){transformElement(slide,'');slide.removeEventListener('click',onOverviewSlideClicked,!0)});toArray(dom.background.querySelectorAll('.slide-background')).forEach(function(background){transformElement(background,'')});transformSlides({overview:''});slide(indexh,indexv);layout();cueAutoSlide();dispatchEvent('overviewhidden',{'indexh':indexh,'indexv':indexv,'currentSlide':currentSlide})}}
...
function pause(){if(config.pause){var wasPaused=dom.wrapper.classList.contains('paused');cancelAutoSlide();dom.wrapper.classList.add('paused');if(wasPaused===!1){dispatchEvent('paused')}}}
...
dom.statusDiv.textContent=getStatusText(currentSlide);updateControls();updateProgress();updateBackground();updateParallax();updateSlideNumber();updateNotes();writeURL();cueAutoSlide()}
function sync(){removeEventListeners();addEventListeners();layout();autoSlide=config.autoSlide;cueAutoSlide();createBackgrounds();writeURL();sortAllFragments();updateControls();updateProgress();updateSlideNumber();updateSlidesVisibility();updateBackground(!0);updateNotesVisibility();updateNotes();formatEmbeddedContent();if(config.autoPlayMedia===!1){stopEmbeddedContent(currentSlide,{unloadIframes:!1})}
...
function updateSlides(selector,index){var slides=toArray(dom.wrapper.querySelectorAll(selector)),slidesLength=slides.length;if(slidesLength){if(config.loop){index%=slidesLength;if(index<0){index=slidesLength+index}}
...
function updateNotes(){if(config.showNotes&&dom.speakerNotes&&currentSlide){dom.speakerNotes.innerHTML=getSlideNotes()||'<span class="notes-placeholder">No notes on this slide.</span>'}}
...
function updateSlideNumber(){if(config.slideNumber&&dom.slideNumber){var value=[];var format='h.v';if(typeof config.slideNumber==='string'){format=config.slideNumber}
switch(format){case 'c':value.push(getSlidePastCount()+1);break;case 'c/t':value.push(getSlidePastCount()+1,'/',getTotalSlides());break;case 'h/v':value.push(indexh+1);if(isVerticalSlide())value.push('/',indexv+1);break;default:value.push(indexh+1);if(isVerticalSlide())value.push('.',indexv+1)}
dom.slideNumber.innerHTML=formatSlideNumber(value[0],value[1],value[2])}}
function formatSlideNumber(a,delimiter,b){if(typeof b==='number'&&!isNaN(b)){return'<span class="slide-number-a">'+a+'</span>'+'<span class="slide-number-delimiter">'+delimiter+'</span>'+'<span class="slide-number-b">'+b+'</span>'}
else{return'<span class="slide-number-a">'+a+'</span>'}}
...
if(currentBackground){startEmbeddedContent(currentBackground);var backgroundImageURL=currentBackground.style.backgroundImage||'';if(/\.gif/i.test(backgroundImageURL)){currentBackground.style.backgroundImage='';window.getComputedStyle(currentBackground).opacity;currentBackground.style.backgroundImage=backgroundImageURL}
...
function loadSlide(slide,options){options=options||{};slide.style.display=config.display;toArray(slide.querySelectorAll('img[data-src], video[data-src], audio[data-src]')).forEach(function(element){element.setAttribute('src',element.getAttribute('data-src'));element.setAttribute('data-lazy-loaded','');element.removeAttribute('data-src')});toArray(slide.querySelectorAll('video, audio')).forEach(function(media){var sources=0;toArray(media.querySelectorAll('source[data-src]')).forEach(function(source){source.setAttribute('src',source.getAttribute('data-src'));source.removeAttribute('data-src');source.setAttribute('data-lazy-loaded','');sources+=1});if(sources>0){media.load()}});var indices=getIndices(slide);var background=getSlideBackground(indices.h,indices.v);if(background){background.style.display='block';if(background.hasAttribute('data-loaded')===!1){background.setAttribute('data-loaded','true');var backgroundImage=slide.getAttribute('data-background-image'),backgroundVideo=slide.getAttribute('data-background-video'),backgroundVideoLoop=slide.hasAttribute('data-background-video-loop'),backgroundVideoMuted=slide.hasAttribute('data-background-video-muted'),backgroundIframe=slide.getAttribute('data-background-iframe');if(backgroundImage){background.style.backgroundImage='url('+backgroundImage+')'}
...
if(backgroundVideoMuted){video.muted=!0}
if(isMobileDevice){video.muted=!0;video.autoplay=!0;video.setAttribute('playsinline','')}
backgroundVideo.split(',').forEach(function(source){video.innerHTML+='<source src="'+source+'">'});background.appendChild(video)}
else if(backgroundIframe&&options.excludeIframes!==!0){var iframe=document.createElement('iframe');iframe.setAttribute('allowfullscreen','');iframe.setAttribute('mozallowfullscreen','');iframe.setAttribute('webkitallowfullscreen','');if(/autoplay=(1|true|yes)/gi.test(backgroundIframe)){iframe.setAttribute('data-src',backgroundIframe)}
else{iframe.setAttribute('src',backgroundIframe)}
iframe.style.width='100%';iframe.style.height='100%';iframe.style.maxHeight='100%';iframe.style.maxWidth='100%';background.appendChild(iframe)}}}}
function unloadSlide(slide){slide.style.display='none';var indices=getIndices(slide);var background=getSlideBackground(indices.h,indices.v);if(background){background.style.display='none'}
toArray(slide.querySelectorAll('video[data-lazy-loaded][src], audio[data-lazy-loaded][src]')).forEach(function(element){element.setAttribute('data-src',element.getAttribute('src'));element.removeAttribute('src')});toArray(slide.querySelectorAll('video[data-lazy-loaded] source[src], audio source[src]')).forEach(function(source){source.setAttribute('data-src',source.getAttribute('src'));source.removeAttribute('src')})}
...
else{return{prev:!1,next:!1}}}
function formatEmbeddedContent(){var _appendParamToIframeSource=function(sourceAttribute,sourceURL,param){toArray(dom.slides.querySelectorAll('iframe['+sourceAttribute+'*="'+sourceURL+'"]')).forEach(function(el){var src=el.getAttribute(sourceAttribute);if(src&&src.indexOf(param)===-1){el.setAttribute(sourceAttribute,src+(!/\?/.test(src)?'?':'&')+param)}})};_appendParamToIframeSource('src','youtube.com/embed/','enablejsapi=1');_appendParamToIframeSource('data-src','youtube.com/embed/','enablejsapi=1');_appendParamToIframeSource('src','player.vimeo.com/','api=1');_appendParamToIframeSource('data-src','player.vimeo.com/','api=1');if(isMobileDevice){toArray(dom.slides.querySelectorAll('video, audio')).forEach(function(el){el.controls=!0})}}
...
if(horizontalSlide.classList.contains('stack')===!1){pastCount++}}
return pastCount}
...
else if(currentSlide){var url='/';var id=currentSlide.getAttribute('id');if(id){id=id.replace(/[^a-zA-Z0-9\-\_\:\.]/g,'')}
...
function getSlideBackground(x,y){var slide=getSlide(x,y);if(slide){return slide.slideBackgroundElement}
return undefined}
...
if(i<=index){if(!element.classList.contains('visible'))fragmentsShown.push(element);element.classList.add('visible');element.classList.remove('current-fragment');dom.statusDiv.textContent=getStatusText(element);if(i===index){element.classList.add('current-fragment');startEmbeddedContent(element)}}
...
if(autoSlide&&!autoSlidePaused&&!isPaused()&&!isOverview()&&(!Reveal.isLastSlide()||availableFragments().next||config.loop===!0)){autoSlideTimeout=setTimeout(function(){typeof config.autoSlideMethod==='function'?config.autoSlideMethod():navigateNext();cueAutoSlide()},autoSlide);autoSlideStartTime=Date.now()}
...
function pauseAutoSlide(){if(autoSlide&&!autoSlidePaused){autoSlidePaused=!0;dispatchEvent('autoslidepaused');clearTimeout(autoSlideTimeout);if(autoSlidePlayer){autoSlidePlayer.setPlaying(!1)}}}
function resumeAutoSlide(){if(autoSlide&&autoSlidePaused){autoSlidePaused=!1;dispatchEvent('autoslideresumed');cueAutoSlide()}}
function navigateLeft(){if(config.rtl){if((isOverview()||nextFragment()===!1)&&availableRoutes().left){slide(indexh+1)}}
else if((isOverview()||previousFragment()===!1)&&availableRoutes().left){slide(indexh-1)}}
function navigateRight(){hasNavigatedRight=!0;if(config.rtl){if((isOverview()||previousFragment()===!1)&&availableRoutes().right){slide(indexh-1)}}
else if((isOverview()||nextFragment()===!1)&&availableRoutes().right){slide(indexh+1)}}
function navigateUp(){if((isOverview()||previousFragment()===!1)&&availableRoutes().up){slide(indexh,indexv-1)}}
function navigateDown(){hasNavigatedDown=!0;if((isOverview()||nextFragment()===!1)&&availableRoutes().down){slide(indexh,indexv+1)}}
function navigatePrev(){if(previousFragment()===!1){if(availableRoutes().up){navigateUp()}
...
function navigateNext(){hasNavigatedRight=!0;hasNavigatedDown=!0;if(nextFragment()===!1){if(availableRoutes().down){navigateDown()}
else if(config.rtl){navigateLeft()}
else{navigateRight()}}}
function isSwipePrevented(target){while(target&&typeof target.hasAttribute==='function'){if(target.hasAttribute('data-prevent-swipe'))return!0;target=target.parentNode}
return!1}
...
function onDocumentKeyDown(event){if(typeof config.keyboardCondition==='function'&&config.keyboardCondition()===!1){return!0}
var autoSlideWasPaused=autoSlidePaused;onUserInput(event);var activeElementIsCE=document.activeElement&&document.activeElement.contentEditable!=='inherit';var activeElementIsInput=document.activeElement&&document.activeElement.tagName&&/input|textarea/i.test(document.activeElement.tagName);var activeElementIsNotes=document.activeElement&&document.activeElement.className&&/speaker-notes/i.test(document.activeElement.className);if(activeElementIsCE||activeElementIsInput||activeElementIsNotes||(event.shiftKey&&event.keyCode!==32)||event.altKey||event.ctrlKey||event.metaKey)return;var resumeKeyCodes=[66,86,190,191];var key;if(typeof config.keyboard==='object'){for(key in config.keyboard){if(config.keyboard[key]==='togglePause'){resumeKeyCodes.push(parseInt(key,10))}}}
if(isPaused()&&resumeKeyCodes.indexOf(event.keyCode)===-1){return!1}
var triggered=!1;if(typeof config.keyboard==='object'){for(key in config.keyboard){if(parseInt(key,10)===event.keyCode){var value=config.keyboard[key];if(typeof value==='function'){value.apply(null,[event])}
else if(typeof value==='string'&&typeof Reveal[value]==='function'){Reveal[value].call()}
triggered=!0}}}
if(triggered===!1){triggered=!0;switch(event.keyCode){case 80:case 33:navigatePrev();break;case 78:case 34:navigateNext();break;case 72:case 37:navigateLeft();break;case 76:case 39:navigateRight();break;case 75:case 38:navigateUp();break;case 74:case 40:navigateDown();break;case 36:slide(0);break;case 35:slide(Number.MAX_VALUE);break;case 32:isOverview()?deactivateOverview():event.shiftKey?navigatePrev():navigateNext();break;case 13:isOverview()?deactivateOverview():triggered=!1;break;case 58:case 59:case 66:case 86:case 190:case 191:togglePause();break;case 70:enterFullscreen();break;case 65:if(config.autoSlideStoppable)toggleAutoSlide(autoSlideWasPaused);break;default:triggered=!1}}
...
cueAutoSlide()}
function onTouchStart(event){if(isSwipePrevented(event.target))return!0;touch.startX=event.touches[0].clientX;touch.startY=event.touches[0].clientY;touch.startCount=event.touches.length;if(event.touches.length===2&&config.overview){touch.startSpan=distanceBetween({x:event.touches[1].clientX,y:event.touches[1].clientY},{x:touch.startX,y:touch.startY})}}
function onTouchMove(event){if(isSwipePrevented(event.target))return!0;if(!touch.captured){onUserInput(event);var currentX=event.touches[0].clientX;var currentY=event.touches[0].clientY;if(event.touches.length===2&&touch.startCount===2&&config.overview){var currentSpan=distanceBetween({x:event.touches[1].clientX,y:event.touches[1].clientY},{x:touch.startX,y:touch.startY});if(Math.abs(touch.startSpan-currentSpan)>touch.threshold){touch.captured=!0;if(currentSpan<touch.startSpan){activateOverview()}
else{deactivateOverview()}}
event.preventDefault()}
else if(event.touches.length===1&&touch.startCount!==2){var deltaX=currentX-touch.startX,deltaY=currentY-touch.startY;if(deltaX>touch.threshold&&Math.abs(deltaX)>Math.abs(deltaY)){touch.captured=!0;navigateLeft()}
else if(deltaX<-touch.threshold&&Math.abs(deltaX)>Math.abs(deltaY)){touch.captured=!0;navigateRight()}
else if(deltaY>touch.threshold){touch.captured=!0;navigateUp()}
else if(deltaY<-touch.threshold){touch.captured=!0;navigateDown()}
...
function onTouchEnd(event){touch.captured=!1}
...
function onPageVisibilityChange(event){var isHidden=document.webkitHidden||document.msHidden||document.hidden;if(isHidden===!1&&document.activeElement!==document.body){if(typeof document.activeElement.blur==='function'){document.activeElement.blur()}
document.body.focus()}}
...
function onAutoSlidePlayerClick(event){if(Reveal.isLastSlide()&&config.loop===!1){slide(0,0);resumeAutoSlide()}
else if(autoSlidePaused){resumeAutoSlide()}
else{pauseAutoSlide()}}
function Playback(container,progressCheck){this.diameter=100;this.diameter2=this.diameter/2;this.thickness=6;this.playing=!1;this.progress=0;this.progressOffset=1;this.container=container;this.progressCheck=progressCheck;this.canvas=document.createElement('canvas');this.canvas.className='playback';this.canvas.width=this.diameter;this.canvas.height=this.diameter;this.canvas.style.width=this.diameter2+'px';this.canvas.style.height=this.diameter2+'px';this.context=this.canvas.getContext('2d');this.container.appendChild(this.canvas);this.render()}
...
this.render();if(this.playing){features.requestAnimationFrameMethod.call(window,this.animate.bind(this))}};Playback.prototype.render=function(){var progress=this.playing?this.progress:0,radius=(this.diameter2)-this.thickness,x=this.diameter2,y=this.diameter2,iconSize=28;this.progressOffset+=(1-this.progressOffset)*0.1;var endAngle=(-Math.PI/2)+(progress*(Math.PI*2));var startAngle=(-Math.PI/2)+(this.progressOffset*(Math.PI*2));this.context.save();this.context.clearRect(0,0,this.diameter,this.diameter);this.context.beginPath();this.context.arc(x,y,radius+4,0,Math.PI*2,!1);this.context.fillStyle='rgba( 0, 0, 0, 0.4 )';this.context.fill();this.context.beginPath();this.context.arc(x,y,radius,0,Math.PI*2,!1);this.context.lineWidth=this.thickness;this.context.strokeStyle='rgba( 255, 255, 255, 0.2 )';this.context.stroke();if(this.playing){this.context.beginPath();this.context.arc(x,y,radius,startAngle,endAngle,!1);this.context.lineWidth=this.thickness;this.context.strokeStyle='#fff';this.context.stroke()}
...
this.context.restore()};Playback.prototype.on=function(type,listener){this.canvas.addEventListener(type,listener,!1)};Playback.prototype.off=function(type,listener){this.canvas.removeEventListener(type,listener,!1)};Playback.prototype.destroy=function(){this.playing=!1;if(this.canvas.parentNode){this.container.removeChild(this.canvas)}};Reveal={VERSION:VERSION,initialize:initialize,configure:configure,sync:sync,slide:slide,left:navigateLeft,right:navigateRight,up:navigateUp,down:navigateDown,prev:navigatePrev,next:navigateNext,navigateFragment:navigateFragment,prevFragment:previousFragment,nextFragment:nextFragment,navigateTo:slide,navigateLeft:navigateLeft,navigateRight:navigateRight,navigateUp:navigateUp,navigateDown:navigateDown,navigatePrev:navigatePrev,navigateNext:navigateNext,layout:layout,shuffle:shuffle,availableRoutes:availableRoutes,availableFragments:availableFragments,toggleHelp:toggleHelp,toggleOverview:toggleOverview,togglePause:togglePause,toggleAutoSlide:toggleAutoSlide,isOverview:isOverview,isPaused:isPaused,isAutoSliding:isAutoSliding,isSpeakerNotes:isSpeakerNotes,loadSlide:loadSlide,unloadSlide:unloadSlide,addEventListeners:addEventListeners,removeEventListeners:removeEventListeners,getState:getState,setState:setState,getSlidePastCount:getSlidePastCount,getProgress:getProgress,getIndices:getIndices,getSlides:getSlides,getTotalSlides:getTotalSlides,getSlide:getSlide,getSlideBackground:getSlideBackground,getSlideNotes:getSlideNotes,getPreviousSlide:function(){return previousSlide},getCurrentSlide:function(){return currentSlide},getScale:function(){return scale},getConfig:function(){return config},getQueryHash:function(){var query={};location.search.replace(/[A-Z0-9]+?=([\w\.%-]*)/gi,function(a){query[a.split('=').shift()]=a.split('=').pop()});for(var i in query){var value=query[i];query[i]=deserialize(unescape(value))}
return query},isFirstSlide:function(){return(indexh===0&&indexv===0)},isLastSlide:function(){if(currentSlide){if(currentSlide.nextElementSibling)return!1;if(isVerticalSlide(currentSlide)&&currentSlide.parentNode.nextElementSibling)return!1;return!0}
return!1},isReady:function(){return loaded},addEventListener:function(type,listener,useCapture){if('addEventListener' in window){(dom.wrapper||document.querySelector('.reveal')).addEventListener(type,listener,useCapture)}},removeEventListener:function(type,listener,useCapture){if('addEventListener' in window){(dom.wrapper||document.querySelector('.reveal')).removeEventListener(type,listener,useCapture)}},triggerKey:function(keyCode){onDocumentKeyDown({keyCode:keyCode})},registerKeyboardShortcut:function(key,value){keyboardShortcuts[key]=value}};return Reveal}))
